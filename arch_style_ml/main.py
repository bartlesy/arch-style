...
import arch_style_ml.data_loader.augmentation as module_aug
import arch_style_ml.data_loader.data_loaders as module_data
import arch_style_ml.model.loss as module_loss
import arch_style_ml.model.metric as module_metric
import arch_style_ml.model.model as module_arch
from arch_style_ml.trainer import Trainer
...
def get_instance(
    module: ModuleType, name: str, config: Dict, *args: Any, **kwargs: Any
) -> Any:
    """
    ...
    kwargs : Any
        Keyword arguments that may be overriden ``kwargs`` in ``config``.
    """
    ctor_name = config[name]["type"]
    config_kwargs = config[name]["args"]
    log.info(f"Building: {module.__name__}.{ctor_name}")
    for k in kwargs:
        if k in config_kwargs:
            del kwargs[k]
    return getattr(module, ctor_name)(*args, **config_kwargs, **kwargs)
...
def train(cfg: Dict, resume: str) -> None:
    ...
    data_set = get_instance(module_data, "dataset", cfg, transformer=transforms)
    data_loader = get_instance(module_data, "data_loader", cfg, dataset=data_set)
